...
int joyCallback(const sensor_msgs::Joy::ConstPtr& joy)
{
	...
	{											//axes[axes_emg1] :  NOT USED | NOT USED | 0: Do Nothing
		rtl_act=1;
	...
	// }
  	...
  	while(ros::ok())
  	{
		...
		{ //Mode Switch 
			...
			switch(axes_mode1)
			{
				case -1:
						off_RcOveride = 0;
						mode_srv.request.base_mode=216;
						mode_srv.request.custom_mode="STABILIZED";
		...
		{	
			...
			srv3.request.base_mode=216;
			srv3.request.custom_mode="AUTO.RTL";
			client2.call(srv3);
		...
		else if(land_act == 1 && armed_status == 1)			//LAND
		{
			...
			srv4.request.base_mode=216;
			srv4.request.custom_mode="AUTO.LAND";
			client2.call(srv4);
		...
		else if(no_com==1)
		{
			...
			srv5.request.base_mode=216;
			srv5.request.custom_mode="OFFBOARD";
			client2.call(srv5);
